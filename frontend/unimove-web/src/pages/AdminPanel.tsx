...
export default function AdminPanel() {
  ...
  useEffect(() => {
    ...
    if (section === 'notifications') {
      setLoading(true);
      Promise.all([
        fetch(`${API}/admin/notifications`, { headers: authHeaders() }).then(r => r.json()),
        users.length === 0 ? fetch(`${API}/admin/users`, { headers: authHeaders() }).then(r => r.json()) : Promise.resolve(null)
      ]).then(([notifsData, usersData]) => {
        setAdminNotifs(Array.isArray(notifsData) ? notifsData : []);
        if (usersData) setUsers(Array.isArray(usersData) ? usersData : usersData.data ?? []);
      }).catch(() => toast('Error cargando notificaciones', 'error'))
        .finally(() => setLoading(false));
    }
    if (section === 'vehicles') {
      setLoading(true);
      fetch(`${API}/admin/vehicles`, { headers: authHeaders() })
        .then(r => r.json()).then(d => setVehicles(Array.isArray(d) ? d : d.data ?? []))
  ...
  const deleteVehicle = (v: Vehicle) => openModal({
    title: 'Eliminar vehículo', desc: '¿Eliminar permanentemente este vehículo?',
    target: `${v.brand} ${v.model} (${v.plate})`, confirmLabel: 'Eliminar', type: 'danger',
    onConfirm: async () => {
      const r = await fetch(`${API}/admin/vehicles/${v.id}`, { method: 'DELETE', headers: authHeaders() });
  ...
  const filteredUsers = users
    ...
    .filter(u => !userSearch || q(u.name).includes(q(userSearch)) || q(u.username).includes(q(userSearch)) || q(u.email).includes(q(userSearch)));
  const filteredTravels = travels
    .filter(t => travelFilter === 'all' || t.status === travelFilter)
    .filter(t => !travelSearch || q(t.origin).includes(q(travelSearch)) || q(t.destination).includes(q(travelSearch)) || q(t.driver?.name ?? '').includes(q(travelSearch)));
  const filteredBookings = bookings
    .filter(b => bookingFilter === 'all' || b.status === bookingFilter)
    .filter(b => !bookingSearch || q(b.passenger?.name ?? '').includes(q(bookingSearch)) || q(b.passenger?.username ?? '').includes(q(bookingSearch)));
  const filteredReviews = reviews
    .filter(rv => reviewRatingFilter === 'all' || String(rv.rating) === reviewRatingFilter)
    .filter(rv => !reviewSearch || q(rv.author?.name ?? '').includes(q(reviewSearch)) || q(rv.recipient?.name ?? '').includes(q(reviewSearch)) || q(rv.comment ?? '').includes(q(reviewSearch)));
  const filteredVehicles = vehicles
    .filter(v => !vehicleSearch || q(v.brand).includes(q(vehicleSearch)) || q(v.model).includes(q(vehicleSearch)) || q(v.plate).includes(q(vehicleSearch)) || q(v.owner?.name ?? '').includes(q(vehicleSearch)));
  const filteredNotifs = adminNotifs
    .filter(n => notifReadFilter === 'all' || (notifReadFilter === 'read' ? n.read : !n.read))
    .filter(n => !notifSearch || q(n.text).includes(q(notifSearch)) || q(n.user?.name ?? '').includes(q(notifSearch)));
    ...
    <div className="admin-layout">
      ...
      <div className="admin-main">
        ...
        <div className="admin-content">
              ...
              <div className="admin-filter-bar" style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  <input value={userSearch} onChange={e => setUserSearch(e.target.value)}
                    ...
                    style={{ padding: '5px 10px', borderRadius: 20, border: '1.5px solid #d5ece3', fontSize: '0.78rem', fontFamily: 'inherit', color: '#103B31', minWidth: 160 }} />
                </div>
          ...
          {section === 'travels' && (
            <div className="admin-table-card">
              <div className="admin-filter-bar" style={{ flexWrap: 'wrap', gap: 8 }}>
                ...
                <input value={travelSearch} onChange={e => setTravelSearch(e.target.value)}
                  placeholder="🔍 Buscar origen, destino, conductor…"
                  style={{ padding: '5px 10px', borderRadius: 20, border: '1.5px solid #d5ece3', fontSize: '0.78rem', fontFamily: 'inherit', color: '#103B31', minWidth: 200, marginLeft: 'auto' }} />
              </div>
              <div className="admin-table-scroll">
                <table className="admin-table">
                  <thead>
          ...
          {section === 'reviews' && (
            <div className="admin-table-card">
              <div className="admin-filter-bar" style={{ flexWrap: 'wrap', gap: 8 }}>
                <span className="admin-filter-label">Valoración:</span>
                {[{ val: 'all', lbl: 'Todas' }, { val: '5', lbl: '★★★★★' }, { val: '4', lbl: '★★★★' }, { val: '3', lbl: '★★★' }, { val: '2', lbl: '★★' }, { val: '1', lbl: '★' }].map(({ val, lbl }) => (
                  <button key={val} className={`admin-filter-chip${reviewRatingFilter === val ? ' active' : ''}`}
                    onClick={() => setReviewRatingFilter(val)}>{lbl}</button>
                ))}
                <input value={reviewSearch} onChange={e => setReviewSearch(e.target.value)}
                  placeholder="🔍 Buscar autor, destinatario, comentario…"
                  style={{ padding: '5px 10px', borderRadius: 20, border: '1.5px solid #d5ece3', fontSize: '0.78rem', fontFamily: 'inherit', color: '#103B31', minWidth: 220, marginLeft: 'auto' }} />
              </div>
              <div className="admin-table-scroll">
                <table className="admin-table">
                  <thead>
                    ...
                    {!loading && filteredReviews.length === 0 && (
                      <tr><td colSpan={6}>
                        <div className="admin-empty-state">
                          ...
                          <p>No hay valoraciones en esta categoría.</p>
                        </div>
                      </td></tr>
                    )}
                    {!loading && filteredReviews.map(rv => (
                      <tr key={rv.id}>
          ...
          {section === 'notifications' && (
            <div>
              ...
              <div className="admin-table-card" style={{ padding: '22px 24px', marginBottom: 20 }}>
                ...
                <form onSubmit={sendNotification} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    >
                      <option value="" style={{ fontWeight: 'bold' }}>Enviar a todos los usuarios</option>
              ...
              {/* Notification list */}
              <div className="admin-table-card">
                <div className="admin-filter-bar" style={{ borderBottom: '1px solid #f0ede8', paddingBottom: 14, marginBottom: 0, flexWrap: 'wrap', gap: 8 }}>
                  <span className="admin-filter-label" style={{ fontWeight: 700, color: '#7ba696', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                    Historial ({adminNotifs.length})
                  </span>
                  <div style={{ display: 'flex', gap: 6, marginLeft: 10 }}>
                    {[{ val: 'all', lbl: 'Todas' }, { val: 'read', lbl: 'Leídas' }, { val: 'unread', lbl: 'No leídas' }].map(({ val, lbl }) => (
                      <button key={val} className={`admin-filter-chip${notifReadFilter === val ? ' active' : ''}`}
                        onClick={() => setNotifReadFilter(val)}>{lbl}</button>
                    ))}
                  </div>
          ...
          {section === 'vehicles' && (
            <div className="admin-table-card">
              <div className="admin-filter-bar" style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <input value={vehicleSearch} onChange={e => setVehicleSearch(e.target.value)}
                  placeholder="🔍 Buscar marca, modelo, matrícula, dueño…"
                  style={{ padding: '5px 10px', borderRadius: 20, border: '1.5px solid #d5ece3', fontSize: '0.78rem', fontFamily: 'inherit', color: '#103B31', minWidth: 260 }} />
              </div>
              <div className="admin-table-scroll">
                <table className="admin-table">
                  <thead>
                    <tr><th>#</th><th>Vehículo</th><th>Matrícula</th><th>Asientos</th><th>Propietario</th><th>Registro</th><th>Acciones</th></tr>
                  </thead>
                  <tbody>
                    {loading && <tr><td colSpan={7} style={{ textAlign: 'center', padding: 32, color: '#7ba696' }}>Cargando vehículos…</td></tr>}
                    {!loading && filteredVehicles.length === 0 && (
                      <tr><td colSpan={7}>
                        <div className="admin-empty-state">
                          <div className="admin-empty-icon"><IconScooter size={32} /></div>
                    ...
                    {!loading && filteredVehicles.map(v => (
                      <tr key={v.id}>
                        <td style={{ color: '#a7d7c5', fontWeight: 700 }}>#{v.id}</td>
                        <td style={{ fontWeight: 'bold' }}>{v.brand} {v.model}</td>
                        <td style={{ color: '#103B31', fontFamily: 'monospace', fontSize: '0.9rem' }}>{v.plate}</td>
                        <td style={{ color: '#7ba696' }}>{v.total_seats}</td>
                        <td>
                          {v.owner ? (
                            <div className="admin-user-cell">
                              <div className="admin-user-avatar" style={{ width: 28, height: 28, fontSize: '0.7rem' }}>{initials(v.owner.name)}</div>
                              <span style={{ fontSize: '0.82rem', color: '#103B31' }}>{v.owner.name} <span style={{ color: '#a7d7c5' }}>@{v.owner.username}</span></span>
                            </div>
                          ) : <span style={{ color: '#a7d7c5' }}>—</span>}
                        </td>
                        <td style={{ color: '#7ba696', fontSize: '0.8rem' }}>{new Date(v.created_at).toLocaleDateString('es-ES')}</td>
                        <td>
                          <button className="admin-action-btn danger" onClick={() => deleteVehicle(v)}>
                            <IconTrash size={14} /> Eliminar
                          </button>
                        </td>
                      </tr>
